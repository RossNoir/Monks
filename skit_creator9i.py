...
class SkitCreator(QMainWindow):
    def __init__(self):
        ...
        # --- AI Generator Tab ---
        self.ai_generator_tab = QWidget()
        ai_layout = QVBoxLayout(self.ai_generator_tab)
        ...
        batch_files_layout = QVBoxLayout(self.batch_file_tab)
        
        batch_group_box = QGroupBox("Load Scripts From a Single File")
        batch_box_layout = QVBoxLayout()
        load_file_button = QPushButton("Load Script File (.txt, .json)...")
        load_file_button.clicked.connect(self.load_single_script_file)
        self.loaded_file_label = QLabel("No file loaded.")
        batch_box_layout.addWidget(load_file_button)
        batch_box_layout.addWidget(self.loaded_file_label)
        batch_box_layout.addStretch()
        batch_group_box.setLayout(batch_box_layout)
        batch_files_layout.addWidget(batch_group_box)

        self.tabs.addTab(self.ai_generator_tab, "AI Generator")
        self.tabs.addTab(self.manual_script_tab, "Manual Script")
        self.tabs.addTab(self.batch_file_tab, "Batch From File")
    ...
    def load_single_script_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Select Script File", "", "Script Files (*.txt *.json)")
        if file:
            self.batch_script_file = file
            self.loaded_file_label.setText(f"Loaded: {os.path.basename(file)}")
            print(f"Loaded single script file for batch processing: {file}")
    ...
    def load_laugh_tracks(self):
        ...
        if self.laugh_tracks:
            # random.shuffle(self.laugh_tracks) # Cycle in order instead of random
    ...
    def generate_skit_batch(self):
        ...
        if current_tab == self.ai_generator_tab:
            ...
            for i in range(batch_count):
                ...
                if parsed_script: scripts_to_process.append(parsed_script)
                    ...
                    break
        elif current_tab == self.manual_script_tab:
            manual_script_text = self.manual_script_input.toPlainText().strip()
            if manual_script_text:
                print("\n--- Using Manual Script Input ---")
                parsed_script = self.parse_raw_script(manual_script_text)
                if parsed_script: scripts_to_process.append(parsed_script)
        elif current_tab == self.batch_file_tab:
            if not self.batch_script_file:
                self.show_message("No File", "Please load a script file to process.")
            else:
                print(f"\n--- Processing Scripts from {os.path.basename(self.batch_script_file)} ---")
                raw_scripts = self.parse_batch_file(self.batch_script_file)
                for i, script_text in enumerate(raw_scripts):
                    print(f"  - Parsing script {i+1} from file...")
                    parsed_script = self.parse_raw_script(script_text)
                    if parsed_script: scripts_to_process.append(parsed_script)
        ...
        try:
            ...
            if file_path.lower().endswith('.json'):
                ...
                if isinstance(scripts, list):
                    ...
                    self.show_message("JSON Error", "JSON file must contain a list of script strings.")
                    return []
            else: # Assume .txt
                return content.split('--- SCRIPT BREAK ---')
        except Exception as e:
            self.show_message("File Error", f"Could not read or parse the batch file:\n{e}")
            return []
