        ...
        for i in range(batch_count):
            ...
            try:
                ...
                if dialogue_segments:
                    # Join segments with a pause in between, but not at the very end
                    full_dialogue_no_delay = dialogue_segments[0]
                    for seg in dialogue_segments[1:]:
                        full_dialogue_no_delay += pause + seg
    ...
    def generate_seinfeld_skit(self):
        ...
        try:
            ...
            for line in full_script.split('\n'):
                ...
                if match:
                    # Sanitize the dialogue line to remove parenthetical directions like (sighs)
                    original_line = match.group(2).strip()
                    sanitized_line = re.sub(r'\s*\([^)]*\)\s*', ' ', original_line).strip()
                    parsed_dialogue.append({"speaker": match.group(1), "line": sanitized_line})
