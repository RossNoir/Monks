...
# <<< NEW >>> Laugh Track File Configuration
LAUGH_FILES = {
...
class SkitCreator(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 750, 780)

        self.f5_model = None
        self.voice_transcripts = {}
        self.ollama_models = []
        self.skit_prompt = ""
        self.laugh_tracks = {} # <<< NEW >>> To hold loaded laugh track audio

        self.load_or_create_prompt_config()
        if F5TTS:
            self.manage_voice_transcripts()
        
        self.load_laugh_tracks() # <<< NEW >>> Load laugh tracks on startup
    ...
    # <<< NEW >>> Function to load laugh tracks into memory
    ...
    def generate_skit_batch(self):
        ...
        for i in range(batch_count):
            ...
            # <<< NEW >>> Generate all audio events (dialogue and laughs)
            audio_events = []
            temp_files_to_clean = []
            ...
            # <<< NEW >>> Assemble audio with laugh track overlays
            try:
                ...
                # Build the main dialogue and laugh track
                dialogue_track = AudioSegment.empty()
                for event in audio_events:
                    if event['type'] == 'dialogue':
                        dialogue_track += event['segment'] + pause
                    elif event['type'] == 'laugh':
                        # Lower volume and overlay at the current end of the dialogue track
                        quiet_laugh = event['segment'] - 8 
                        dialogue_track = dialogue_track.overlay(quiet_laugh)
    ...
    def generate_seinfeld_skit(self): # <<< UPDATED >>> Parses for laugh cues
        ...
        try:
            ...
            for line in full_script.split('\n'):
                ...
                laugh_short_match = re.match(r'\[LAUGH_SHORT\]', line, re.IGNORECASE)
                laugh_long_match = re.match(r'\[LAUGH_LONG\]', line, re.IGNORECASE)

                if dialogue_match:
                    original_line = dialogue_match.group(2).strip()
                    sanitized_line = re.sub(r'\s*\([^)]*\)\s*', ' ', original_line).strip()
                    if sanitized_line: # Only add if there's actual dialogue left
                        parsed_script.append({"type": "dialogue", "speaker": dialogue_match.group(1), "line": sanitized_line})
                elif laugh_short_match:
                    parsed_script.append({"type": "laugh", "duration": "short"})
                elif laugh_long_match:
