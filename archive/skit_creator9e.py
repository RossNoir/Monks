...
class SkitCreator(QMainWindow):
    ...
    def generate_skit_batch(self):
        ...
        for i, parsed_script in enumerate(scripts_to_process):
            ...
            try:
                self.assemble_and_save_audio(current_batch_num, parsed_script, audio_events, intro_file, ender_file, dialogue_delay_sec)
            finally:
                for f in temp_files_to_clean:
                    try: os.remove(f)
                    except OSError: pass
    ...
    def assemble_and_save_audio(self, skit_num, parsed_script, audio_events, intro_file, ender_file, delay_sec):
        try:
            print(f"Assembling audio for skit {skit_num} using master timeline method...")
            intro_music_bed = AudioSegment.from_mp3(intro_file)
            ender_segment = AudioSegment.from_mp3(ender_file)
            pause = AudioSegment.silent(duration=300)

            # --- CORRECTED AUDIO ASSEMBLY LOGIC: MASTER TIMELINE METHOD ---
            
            # This track will be our final mix. We will place audio onto it at specific times.
            final_dialogue_track = AudioSegment.empty()
            
            # The cursor tracks where the *next* line of dialogue should begin.
            cursor = 0
            
            # Get an iterator for the generated dialogue audio segments.
            audio_segment_iter = iter(e['segment'] for e in audio_events if e['type'] == 'dialogue')

            for event in parsed_script:
                if event['type'] == 'dialogue':
                    try:
                        dialogue_segment = next(audio_segment_iter)
                    except StopIteration:
                        print("ERROR: Mismatch between parsed script and generated audio events.")
                        break

                    # Place the current dialogue line at the cursor position.
                    final_dialogue_track = final_dialogue_track.overlay(dialogue_segment, position=cursor)

                    if event['has_laugh']:
                        laugh_segment = next(self.laugh_cycle) - 4
                        
                        # **NEW TIMING**: Start the laugh 60% of the way through the dialogue line.
                        # This creates a much more natural overlay effect.
                        laugh_start_pos = cursor + (len(dialogue_segment) * 0.6)
                        
                        # Overlay the laugh. If it's long, the next dialogue will just play over its tail end.
                        final_dialogue_track = final_dialogue_track.overlay(laugh_segment, position=laugh_start_pos)

                    # **CRITICAL FIX**: Advance the cursor based ONLY on the dialogue and pause length.
                    # The length of the laugh track is ignored, so the dialogue never pauses.
                    cursor += len(dialogue_segment) + len(pause)

                elif event['type'] == 'laugh': # This handles [LAUGH] on its own line
                    laugh_segment = next(self.laugh_cycle) - 4
                    # Place the standalone laugh where the dialogue would have started.
                    final_dialogue_track = final_dialogue_track.overlay(laugh_segment, position=cursor)
                    # We DON'T advance the cursor here. The next dialogue line will start after its
                    # normal pause, creating a seamless transition.
            ...
            # Trim the final dialogue track to the final cursor position to ensure a clean ending
            full_dialogue = full_dialogue[:len(initial_delay) + cursor]
            ...
            print(f"  -> Successfully assembled and saved skit to '{save_path}'")
        except Exception as e:
    ...
    def _parse_script_lines(self, lines):
        ...
        for line in lines:
            ...
            # Find dialogue, which must start the line
            ...
            if dialogue_match:
                ...
                # Check for a laugh cue within the content
                laugh_match = re.search(r'\[LAUGH\]', content, re.IGNORECASE)
                
                # Clean the line for TTS (remove laughs and parentheticals)
                text_to_speak = re.sub(r'\s*\([^)]*\)\s*', ' ', content)
                text_to_speak = re.sub(r'\s*\[LAUGH\]\s*', ' ', text_to_speak, flags=re.IGNORECASE).strip()
                
                if text_to_speak:
                    # Associate the laugh directly with the dialogue line
                    parsed_script.append({
                        "type": "dialogue", 
                        "speaker": speaker, 
                        "line": text_to_speak,
                        "has_laugh": laugh_match is not None
                    })
            
            # Handle lines that are ONLY a laugh cue (these will play sequentially)
            elif re.match(r'^\s*\[LAUGH\]\s*$', line, re.IGNORECASE):
                parsed_script.append({"type": "laugh"})

        print("--- Parsed Script Events ---")
        for item in parsed_script:
            detail = item.get('line') or 'N/A'
            has_laugh_info = f", Has Laugh: {item.get('has_laugh')}" if item['type'] == 'dialogue' else ""
            print(f"  - Type: {item['type']:<10} | Detail: {detail}{has_laugh_info}")
        print("----------------------------")
