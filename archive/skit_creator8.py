            ...
            try:
                ...
                for event in audio_events:
                    if event['type'] == 'dialogue':
                        ...
                        quiet_laugh = event['segment'] - 16
                        # Ensure we don't try to overlay on a negative position
                        laugh_start_pos = len(dialogue_track) - len(pause)
                        if laugh_start_pos < 0:
                            laugh_start_pos = 0
                        # Overlay the laugh starting at the end of the last dialogue segment
                        dialogue_track = dialogue_track.overlay(quiet_laugh, position=laugh_start_pos)
    ...
    def generate_seinfeld_skit(self):
        ...
        try:
            ...
            for line in full_script.split('\n'):
                ...
                # Find laugh cues first
                laugh_short_match = re.search(r'\[LAUGH_SHORT\]', line, re.IGNORECASE)
                laugh_long_match = re.search(r'\[LAUGH_LONG\]', line, re.IGNORECASE)
                
                # Find dialogue, which must start the line
                dialogue_match = re.match(r'^(' + '|'.join(CHARACTERS) + r'):\s*(.*)', line)

                if dialogue_match:
                    # If there's dialogue, extract it and clean it of any cues
                    original_line = dialogue_match.group(2).strip()
                    text_to_speak = re.sub(r'\s*\([^)]*\)\s*', ' ', original_line)
                    text_to_speak = re.sub(r'\s*\[LAUGH_(?:SHORT|LONG)\]\s*', ' ', text_to_speak, flags=re.IGNORECASE).strip()
                    
                    if text_to_speak:
                        parsed_script.append({"type": "dialogue", "speaker": dialogue_match.group(1), "line": text_to_speak})

                # Now, regardless of dialogue, add any laugh cue found on the line
                if laugh_long_match:
