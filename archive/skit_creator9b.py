...
class SkitCreator(QMainWindow):
    def __init__(self):
        ...
        instruction_label = QLabel("Format: CHARACTER: Dialogue line. Use [LAUGH_SHORT] or [LAUGH_LONG] on new lines or at the end of dialogue lines.")
        ...
        try:
            ...
            for event in audio_events:
                if event['type'] == 'dialogue':
                    timeline.append({'type': 'dialogue', 'segment': event['segment'], 'position': current_time})
                    current_time += len(event['segment']) + len(pause)
                elif event['type'] == 'laugh':
                    laugh_position = max(0, current_time - len(pause) - 500) # 500ms earlier
                    timeline.append({'type': 'laugh', 'segment': event['segment'], 'position': laugh_position})
            ...
            for item in timeline:
                if item['type'] == 'dialogue':
                    dialogue_track = dialogue_track.overlay(item['segment'], position=item['position'])
                elif item['type'] == 'laugh':
                    quiet_laugh = item['segment'] - 8
                    dialogue_track = dialogue_track.overlay(quiet_laugh, position=item['position'])
    ...
    def _parse_script_lines(self, lines):
        ...
        for item in parsed_script:
            detail = item.get('line') or item.get('duration')
